...
def reset():
    ...
    startTime = t.time()
    gameOver = False
    snake = [[0,-20],[0, 0], [0, 20], [0, 40], [0, 60], [0, 80],[0, 100],
             [0, 120],[0, 140]]
    snake_dir = "up"
...
def food_collision():
    global food1, trap1
    if get_distance(snake[-1], food1['position']) < 20:
        food_goto_random()
        trap_goto_random()
        ...
        for _ in range(100):
            ...
            for snakePart in snake[:-1]:
                if get_distance(snakePart, food1['position']) < 20:
                    print('ON')
                    food_goto_random()
                    iscollision = True
            if not iscollision:
...
def start():
    global w, h, food_size, delay, startTime, score, offsets
